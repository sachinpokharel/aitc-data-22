...
const International = async () => {
  const data = await fetchData<CommonSubServiceSection>('/services/it-support/section-two');
  return (
    <CommonAnimation className='bg-lightShad2 '>
      <div className='inter-national main-common-padding flex max-w-[1350px] flex-col items-center gap-[3.25rem]  '>
        <CommonTitle title={data?.title as string} className='text-center' />
        <div className='grid w-fit grid-cols-1 place-items-center items-start gap-x-8 gap-y-10 sm:grid-cols-2 xl:grid-cols-6'>
          {data?.keyPoints?.map((item, index) => (
            <div
              className={cn(
                'col-span-2 flex flex-col items-center justify-start gap-2 sm:max-w-[310px] ',
            ...
            >
              ...
              <div className='flex flex-col gap-4'>
                ...
                <CommonParargraph text={item?.description as string} className='text-center !text-base' />
              </div>
            </div>
          ))}
        </div>
