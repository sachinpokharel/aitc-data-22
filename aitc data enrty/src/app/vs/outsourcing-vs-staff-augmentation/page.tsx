...
import {
  ...
  whyAITCCards,
  faqs,
} from '@/data/vs/outsourcing-vs-staff-augmentation-data';

export default function OutsourcingVsStaffAugmentationPage() {
    ...
    <>
      ...
      <section className='common-padding bg-white py-16 md:py-24'>
        <div className='mx-auto max-w-5xl'>
          ...
          <h2 className='mb-4 text-center text-3xl font-bold text-secondary sm:text-4xl'>Which Model Fits Your Needs?</h2>
          ...
          <div className='grid gap-6 md:grid-cols-2'>
            {quickAnswerCards.map((card, i) => (
              ...
              >
                <h3 className={`mb-1 text-xl font-bold ${i === 0 ? 'text-white' : 'text-secondary'}`}>
                  {card.model}
                </h3>
                <p className={`mb-5 text-sm font-medium ${i === 0 ? 'text-white/70' : 'text-primary'}`}>
                  {card.tagline}
                </p>
                <ul className='mb-6 space-y-2'>
                  {card.bestFor.map((item) => (
                    <li key={item} className={`flex items-center gap-2 text-sm ${i === 0 ? 'text-white/90' : 'text-mainBlack'}`}>
                      <span className={i === 0 ? 'text-white' : 'text-primary'}>✓</span> {item}
                    </li>
                  ))}
                </ul>
                <Link
              ...
              <thead>
                <tr>
                  ...
                  <th className='bg-secondary px-6 py-4 text-left text-sm font-bold text-white'>
                    Outsourcing
                    <span className='ml-2 rounded bg-primary px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white'>
                      Managed
                    </span>
                  </th>
                ...
                {featureBreakdown.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className='border-t border-lightShade1'>
                      ...
                      <td className='bg-secondary px-6 py-4 text-sm text-white/90'>{row.outsourcing}</td>
        ...
        <div className='mx-auto max-w-6xl'>
          ...
          <div className='grid gap-8 md:grid-cols-2'>
            ...
            <div className='rounded-2xl border border-secondary/20 bg-secondary p-8 text-white'>
              ...
              <h3 className='mb-1 text-lg font-bold text-white'>End-to-End Delivery Model</h3>
              <p className='mb-6 text-sm text-white/60'>Fully managed execution from idea to deployment</p>
            ...
            <div className='rounded-2xl border border-lightShade1 bg-lightShad2 p-8'>
              ...
              <h3 className='mb-1 text-lg font-bold text-secondary'>Team Extension Model</h3>
              <p className='mb-6 text-sm text-grey'>External developers integrated into your internal workflow</p>
              <ol className='space-y-4'>
      ...
      <section className='common-padding bg-white py-16 md:py-24'>
        <div className='mx-auto max-w-6xl'>
          ...
          <h2 className='mb-4 text-3xl font-bold text-secondary sm:text-4xl'>Which Model Would We Recommend?</h2>
          <p className='mb-10 max-w-2xl text-grey'>
        ...
        <div className='mx-auto max-w-3xl'>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-lg border px-4 py-3 text-left text-sm font-medium transition-all ${
                          selected
                            ? opt.recommendation === 'Outsourcing'
          ...
          {decisionResult && (
            <div className={`mt-8 rounded-2xl border-2 p-8 text-center ${
              decisionResult === 'Outsourcing'
                ? 'border-secondary bg-secondary/5'
                : 'border-primary bg-primary/5'
            }`}>
              <p className='text-xs font-bold uppercase tracking-widest text-grey'>Our Recommendation</p>
              <p className={`mt-2 text-2xl font-bold ${decisionResult === 'Outsourcing' ? 'text-secondary' : 'text-primary'}`}>
                {decisionResult}
              </p>
              <p className='mt-2 text-sm text-grey'>
                {decisionResult === 'Outsourcing'
                  ? 'A fully managed team will give you the fastest path to delivery without internal hiring overhead.'
                  : 'Adding skilled developers to your existing team is the most efficient way to scale delivery.'}
              </p>
              <div className='mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row'>
                <Link
                  href={decisionResult === 'Outsourcing' ? '/request-a-quote' : '/hire/hire-dedicated-developers'}
                  className={`rounded-md px-6 py-3 font-semibold text-white transition-opacity hover:opacity-90 ${
                    decisionResult === 'Outsourcing' ? 'bg-secondary' : 'bg-primary'
                  }`}
                >
                  {decisionResult === 'Outsourcing' ? 'Get a Project Proposal →' : 'Hire Developers →'}
                </Link>
                <button
                  onClick={handleReset}
      ...
      <section className='common-padding bg-lightShad2 py-16 md:py-24'>
        <div className='mx-auto max-w-6xl'>
          ...
          <div className='mt-12 flex flex-col items-center justify-between gap-6 rounded-2xl border border-secondary/20 bg-secondary p-8 text-white sm:flex-row'>
            <div>
              <p className='text-xl font-bold'>Ready to choose the right engagement model?</p>
              <p className='mt-1 text-sm text-white/70'>We evaluate your needs and match the right model — then deliver it.</p>
            </div>
      ...
      <BottomCTAForm
        ...
        heading='Not Sure Whether to Outsource or Extend Your Team?'
        description='Get a clear recommendation based on your project scope, internal team structure, and long-term development needs.'
        bullets={[
        ...
        form={{
          formName: 'outsourcing-vs-staff-aug-cta',
          heading: 'Tell Us About Your Project',
          subheading: "We'll recommend the right engagement model and outline exactly how we'd approach it.",
