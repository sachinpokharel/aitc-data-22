...
const MainHeader = ({ data, showHiring }: { data: ServiceCategory[]; showHiring: boolean }) => {
  const [showServices, setShowServices] = useState<boolean>(false);
  const pathname = usePathname();

  return (
    <div className='w-full'>
      <nav className='header relative  flex w-full flex-row items-center justify-between  border-t-2 border-lightShade1 max-midlg:py-2.5 max-xmd:px-6 xmd:flex-row  xmd:justify-between xmd:px-12 xl:px-[6.25rem] 3xl:px-[9.375rem]  '>
        <Link href='/' className='flex h-full items-center justify-center gap-4'>
          <svg width='41.4' height='52' viewBox='0 0 48 61' fill='none' xmlns='http://www.w3.org/2000/svg'>
            <mask
              id='mask0_5045_66571'
          ...
          <div className='iso-info border-l-2 border-[#E5E5FF] px-3 text-[18px]  text-secondary'>
            <p className='font-black'>ISO 9001 : 2015</p>
            <p className='font-medium'>Certified</p>
          </div>
        </Link>
        <div className='hidden h-full flex-row items-center gap-12  midlg:flex midlg:gap-10 xl:gap-9 2xl:gap-14 3xl:gap-16'>
          <Link className={`text-lg ${pathname === '/aboutus' ? 'text-primary' : ' text-mainBlack'}`} href='/aboutus'>
            About
          </Link>
          <Link className={`text-lg  ${pathname === '/why-us' ? 'text-primary' : ' text-mainBlack'} `} href='/why-us'>
            Why Us
          </Link>
          <div
            className='h-full  py-6  text-lg text-mainBlack '
            onMouseEnter={(e) => {
              e.preventDefault();
          ...
          <Link className={`text-lg  ${pathname === '/ourwork' ? 'text-primary' : ' text-mainBlack'} `} href='/ourwork'>
            Our Work
          </Link>

          <Link className={`text-lg  ${pathname === '/career' ? 'text-primary' : ' text-mainBlack'} `} href='/career'>
            Career
          </Link>

          <Link
            className={`text-lg  ${pathname === '/insights' ? 'text-primary' : ' text-mainBlack'} `}
            href='/insights'
          >
            Insights
          </Link>
          ...
          <Link
            className={`text-lg  ${pathname === '/education' ? 'text-primary' : ' text-mainBlack'} `}
          ...
          {/* <Link
              className={`text-lg  ${pathname === '/buy-domain' ? 'text-primary' : ' text-mainBlack'} `}
              href='/buy-domain'
            >
              Buy Domain
            </Link> */}
          {/* <Link
              className={`text-lg  ${pathname === '/corporate-social-responsibility' ? 'text-primary' : ' text-mainBlack'} `}
        ...
        <MobileLayout data={data} showHiring={showHiring} />
      </nav>

      <div className='relative w-full'>
        {showServices && <DesktopServices setShowServices={setShowServices} data={data} />}
      </div>
    </div>
  );
};
