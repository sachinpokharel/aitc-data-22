...
// px of scroll distance allocated per tab when stickyScroll is on
const STEP_PX = 220;
...
export default function Solutions({
  ...
  // Set the wrapper height so that there is (tabs.length * STEP_PX) px of
  // scroll space beyond the viewport — desktop only, auto on mobile.
  useEffect(() => {
    ...
    const updateHeight = () => {
      container.style.height = isDesktop() ? `calc(100vh + ${tabs.length * STEP_PX}px)` : 'auto';
    };

    updateHeight();
    window.addEventListener('resize', updateHeight);
    return () => window.removeEventListener('resize', updateHeight);
  }, [stickyScroll, tabs.length]);

  // Drive the active tab from scroll position while the section is pinned.
  useEffect(() => {
    if (!stickyScroll) return;
    const container = containerRef.current;
    if (!container) return;
    ...
    const onScroll = () => {
      if (!isDesktop()) return;
      // Absolute top of the container from the document top
      const containerTop = container.getBoundingClientRect().top + window.scrollY;
      const progress = (window.scrollY - containerTop) / (tabs.length * STEP_PX);
      setActive(Math.min(tabs.length - 1, Math.max(0, Math.floor(progress * tabs.length))));
    };

    window.addEventListener('scroll', onScroll, { passive: true });
    onScroll(); // sync on mount in case page was already scrolled
  ...
  const section = (
    <section
      id='solutions'
      className={`py-12 sm:py-16 md:py-20${stickyScroll ? ' lg:sticky lg:top-0 lg:min-h-screen' : ''} ${sectionClassName}`}
    >
      <div className='mx-auto max-w-7xl px-4 sm:px-6'>
        ...
        <div className='hidden lg:flex lg:flex-row lg:gap-0 xl:min-h-[480px]'>
          {/* Tab list */}
  ...
  if (stickyScroll) {
    return <div ref={containerRef}>{section}</div>;
  }
