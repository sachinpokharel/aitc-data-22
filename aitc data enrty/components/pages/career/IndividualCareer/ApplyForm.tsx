...
type IApplyFormData = yup.InferType<typeof applySchema>;

const ApplyForm = ({ vacancyId, alternate }: { vacancyId?: number; alternate?: boolean }) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const onSubmit = async (data: IApplyFormData) => {
    try {
      ...
      const endpoint = alternate ? '/career/interest-based-application' : '/career/vacancy-response';
