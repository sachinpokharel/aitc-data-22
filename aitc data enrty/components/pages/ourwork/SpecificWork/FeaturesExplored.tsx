...
const FeaturesExplored = async ({ id }: { id: string }) => {
  ...
  return (
    <div className={cn(`common-padding common-section flex flex-col gap-12`, id && 'bg-lightShad2')}>
      ...
      <div className='flex flex-wrap gap-8 max-xmd:gap-10 2xxl:w-2/3'>
        {data?.endpoint?.map((item) => (
          <div
            key={item?.id}
            className='group mx-auto flex w-[293px] flex-row items-center gap-4 rounded-lg p-4 [background:linear-gradient(90deg,#FFF_6%,#EDEDFF_79%)]'
          >
            <Image
              src={`/icons/arrow.png`}
              alt='features'
            ...
            />
            <p className='text-lg text-mainBlack transition-all duration-300 group-hover:translate-x-2'>
