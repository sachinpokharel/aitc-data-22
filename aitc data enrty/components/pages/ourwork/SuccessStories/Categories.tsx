...
const Categories = ({ name, slug }: { name: string; slug: string }) => {
  ...
  return (
    <button
      onClick={() => {
        ...
        slug === 'all' ? optional.delete('tab') : optional.set('tab', slug === 'all' ? '' : slug);
      ...
      className={cn(
        `w-fit rounded-md bg-lightShad2  px-4 py-3 transition-all  duration-300`,
        (tab || 'all') === slug ? 'border border-primary text-mainBlack' : 'border border-transparent  text-grey'
      )}
    >
