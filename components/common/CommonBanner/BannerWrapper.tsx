...
const BannerWrapper = ({ children, image, className }: IProps) => {
  return (
    <section className={cn('flex flex-col  gap-4 bg-lightShad2 xl:flex-row', className)}>
      <div className='flex flex-col items-center justify-center gap-12 px-6 py-10 xmd:px-16 xmd:py-[3.25rem] xl:w-1/2 3xl:px-[9.375rem] '>
        {children}
      </div>
      {/* large screen image or svg 1280 */}
      <span className='hidden xl:block xl:w-1/2'>
        <svg width='100%' height='597' fill='none' xmlns='http://www.w3.org/2000/svg'>
          <defs>
      ...
      <span className='block w-full xl:hidden'>
        <svg xmlns='http://www.w3.org/2000/svg' width='100%' height='100%' viewBox='0 0 834 529'>
          <defs>
...
export const BannerTextWrapper = ({ children }: IProps) => {
  return (
